...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/10 23:32:06 by mdziadko          #+#    #+#             */
/*   Updated: 2025/08/11 13:34:57 by mdziadko         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	add_new_env_var(t_env **env_list_head, char *key, char *value)
{
	...
	if (*env_list_head == NULL)
		*env_list_head = new_env(full_str, true);
	else
	{
		current = *env_list_head;
		while (current->next)
			current = current->next;
		current->next = new_env(full_str, true);
	}
	free(full_str);
	return (0);
}
...
/*
int	builtin_env(t_data *mini, char **args)
{
	if (!mini || !args || !*args || ft_strcmp("env", args[0]))
		return (1);
	if (args[1])
		return (report_error("env: No such file or directory", 127));
